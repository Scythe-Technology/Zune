...
pub fn close(self: *Self) void {
    if (self.tls) |*tls| _ = tls.writeAllEnd(self.stream, "", true) catch {};
    self.stream.close();
}

pub fn write(self: *Self, buffer: []const u8) !usize {
    if (self.tls) |*tls| return tls.write(self.stream, buffer);
    return self.stream.write(buffer);
}

pub fn read(self: *Self, buffer: []u8) !usize {
    if (self.tls) |*tls| return tls.read(self.stream, buffer);
    return self.stream.read(buffer);
}

pub fn writeAll(self: *Self, data: []const u8) !void {
    if (self.tls) |*tls| return tls.writeAll(self.stream, data);
