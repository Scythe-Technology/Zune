...
pub fn lua_nextinteger(L: *Luau) i32 {
    ...
    if (min > max)
        L.raiseErrorStr("InvalidRange (min > max)", .{});
...
pub fn lua_fill(L: *Luau) i32 {
    ...
    if (offset < 0)
        L.raiseErrorStr("InvalidOffset (offset < 0)", .{});
    if (length < 0)
        L.raiseErrorStr("InvalidLength (length < 0)", .{});
    if (offset + length > buffer.len)
        L.raiseErrorStr("InvalidLength (offset + length > buffer size)", .{});
